...
host_response <- boston %>%
  ...
  summarise(rate = unique(host_response_rate),
            time = unique(host_response_time))

# find the time when each operator first started hosting Airbnb
# Owners who have been operating airbnb longer are usually
#able to better understand the needs of different users.
host_start <- boston %>%
  group_by(host_id, host_name) %>%
...
# the host, users can estimate the price range of the host
...
# By knowing the property type of each room, users can
#choose their favorite type and avoid the trouble caused by the room types.
